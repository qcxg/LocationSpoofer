...
object TrajectorySimulator {
    ...
    fun calculateSimulatedLocation(
        ...
        jitterRadiusMeters: Double = 30.0
    ): SimulatedLocation {
        ...
        val (speedMs, stepFreqHz, modeJitterRadius) = getModeParams(simModeName)
        val jitterRadius = if (enableJitter) jitterRadiusMeters.coerceIn(1.0, 80.0) else modeJitterRadius
    ...
    private fun applyGaussianDrift(currentTime: Long, jitterRadius: Double) {
        ...
        // sigma controls the per-second OU step. Keep it much smaller than the hard
        // radius so the position keeps refreshing without producing visible jumps.
        val sigma = jitterRadius / 18.0

        // alpha: 均值回归强度，避免累计偏移长时间贴边。
        val alpha = 0.12
        ...
        val driftDistance = sqrt(driftLatMeters * driftLatMeters + driftLngMeters * driftLngMeters)
        if (driftDistance > jitterRadius && driftDistance > 0.0) {
            val scale = jitterRadius / driftDistance
            driftLatMeters *= scale
            driftLngMeters *= scale
        }
    }
